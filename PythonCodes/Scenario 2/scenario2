...
# Great >> v_max = 0.2, v_min = 0.0, omega_max = 1.5, omega_min = -omega_max
v_max = +0.2; v_min = 0.0 # -v_max
omega_max = +1.5; omega_min = -omega_max
...
rhs = np.array([[v1*np.cos(theta1)],[v1*np.sin(theta1)],[omega1], \
[v2*np.cos(theta2)],[v2*np.sin(theta2)],[omega2], \
[v3*np.cos(theta3)],[v3*np.sin(theta3)],[omega3], \
[v4*np.cos(theta4)],[v4*np.sin(theta4)],[omega4]])                   # system r.h.s
# print("rhs: ", rhs)
...
P = SX.sym('P', n_states + N*(n_states + n_controls))                                             # parameters (which include the initial state and the reference state)
# print("U: ", U)
# print("P: ", P)
...
Q[0,0] = 1; Q[1,1] = 5; Q[2,2] = 0.1; Q[3,3] = 1; Q[4,4] = 5; Q[5,5] = 0.1;
Q[6,6] = 1; Q[7,7] = 5; Q[8,8] = 0.1; Q[9,9] = 1; Q[10,10] = 5; Q[11,11] = 0.1   #0.1                           # weighing matrices (states)
R = np.zeros((n_controls,n_controls));
R[0,0] = 0.5; R[1,1] = 0.05; R[2,2] = 0.5; R[3,3] = 0.05;
R[4,4] = 0.5; R[5,5] = 0.05; R[6,6] = 0.5; R[7,7] = 0.05       #0.05                                  # weighing matrices (controls)
...
# g = vertcat(vertcat(vertcat(st-P[0:n_states], np.array([3.5])), np.array([3.5])), np.array([3.5]))                                                                   # initial condition constraints
g = vertcat(st-P[0:n_states], np.matlib.repmat(np.array([3.5]), M, 1))
...
for k in range(N):
    ...
    # obj = obj  +  mtimes((st-P[6:]).T,mtimes(Q,(st-P[6:]))) + mtimes(con.T, mtimes(R, con))               # calculate obj
    obj = obj + mtimes((st-P[(k+1)*n_states+k*n_controls:(k+2)*n_states+k*n_controls]).T,\
    mtimes(Q,(st-P[(k+1)*n_states+k*n_controls:(k+2)*n_states+k*n_controls]))) + \
    mtimes((con-P[(k+2)*n_states+k*n_controls:(k+2)*n_states+(k+1)*n_controls]).T,\
    mtimes(R,(con-P[(k+2)*n_states+k*n_controls:(k+2)*n_states+(k+1)*n_controls])))
    # print("Obj: ", obj)
    #--->
    # st_next = X[:,k+1];
    st_next = X[0:,k+1]
    ...
    g = vertcat(vertcat(vertcat(g, \
    vertcat(vertcat(vertcat(st_next-st_next_euler, d12), d13), d14), d23), d24), d34)
    # compute constraints
...
# 'lbx': np.concatenate((np.matlib.repmat(np.array([[-10],[-10],[-math.pi],[-10],[-10],[-np.Inf],[-10],[-10],[-math.pi]]),N+1,1),np.matlib.repmat(np.array([[v_min],[omega_min],[v_min],[omega_min],[v_min],[omega_min]]),N,1)), axis=0), \
# 'ubx': np.concatenate((np.matlib.repmat(np.array([[+10],[+10],[+math.pi],[+10],[+10],[+np.Inf],[+10],[+10],[+np.Inf]]),N+1,1),np.matlib.repmat(np.array([[v_max],[omega_max],[v_max],[omega_max],[v_max],[omega_max]]),N,1)), axis=0), \
# 'p': np.zeros((n_states + N*(n_states + n_controls),1))}

args = {\
'lbg': np.matlib.repmat(horzcat(np.zeros((1,n_states)), np.matlib.repmat(np.array([dmin*dmin]), 1, M)), 1, N+1), \
'ubg': np.matlib.repmat(horzcat(np.zeros((1,n_states)), np.matlib.repmat(np.array([np.Inf]), 1, M)), 1, N+1), \
'lbx': np.concatenate((np.matlib.repmat(np.array([[-20],[-2.0],[-np.pi/2],[-20],[-2.0],[-np.pi/2],[-20],[-2.0],[-np.pi/2],[-20],[-2.0],[-np.pi/2]]),N+1,1),np.matlib.repmat(np.array([[v_min],[omega_min],[v_min],[omega_min], [v_min],[omega_min], [v_min],[omega_min]]),N,1)), axis=0), \
'ubx': np.concatenate((np.matlib.repmat(np.array([[+20],[+2.0],[+np.pi/2],[+20],[+2.0],[+np.pi/2],[+20],[+2.0],[+np.pi/2],[+20],[+2.0],[+np.pi/2]]),N+1,1),np.matlib.repmat(np.array([[v_max],[omega_max],[v_max],[omega_max],[v_max],[omega_max],[v_max],[omega_max]]),N,1)), axis=0), \
'p': np.zeros((n_states + N*(n_states + n_controls),1))}
# print("args: ", args)
...
sim_tim = 1000
t0 = 0;
x0 = np.array([[-2.0], [0.0], [0.0], [-1.5], [-0.5], [+1.57], [-1.0], [+1.0], [-1.57], [-0.5], [-1.5], [1.57]])
#
Xr1 = np.array([[-2.0], [+0.0], [0.0]])
Xr2 = np.array([[-1.5], [-0.5], [1.57]])                                                            # initial condition.
Xr3 = np.array([[-1.0], [+1.0], [-1.57]])                                                            # initial condition.
Xr4 = np.array([[-0.5], [-1.5], [1.57]])                                                            # initial condition.
...
# print("x0: ", x0)
# xs = np.array([[+1.0], [+1.0], [0.785], [-1.0], [-1.0], [-2.356]])                                                        # Reference posture.
...
while (not rospy.is_shutdown()):
 # and (mpciter < sim_tim / T)
    ...
    # xs = np.array([[+1.0], [+1.0], [0.785], [-1.0], [-1.0], [-2.356]])
    current_time = mpciter*T
    args['p'][0:n_states] = x0
    ...
    for k in range(N):
        ...
        # Straight Line
        if t_predict <= 40.0:
            x_ref1 = -2.0 + 0.15*t_predict; y_ref1 = 0.0; theta_ref1 = 0.0; xdot_ref1 = 0.15; ydot_ref1 = 0.0
            v_ref1 = np.sqrt(np.power(xdot_ref1,2)+np.power(ydot_ref1,2)); omega_ref1 = 0.0
            #
            x_ref2 = -1.5 + 0.15*t_predict; y_ref2 = 0.0; theta_ref2 = 0.0; xdot_ref2 = 0.15; ydot_ref2 = 0.0
            v_ref2 = np.sqrt(np.power(xdot_ref2,2)+np.power(ydot_ref2,2)); omega_ref2 = 0.0
            #
            x_ref3 = -1.0+ 0.15*t_predict; y_ref3 = 0.0; theta_ref3 = 0.0; xdot_ref3 = 0.15; ydot_ref3 = 0.0
            v_ref3 = np.sqrt(np.power(xdot_ref3, 2) + np.power(ydot_ref3,2)); omega_ref3 = 0.0
            #
            x_ref4 = -0.5 + 0.15*t_predict; y_ref4 = 0.0; theta_ref4 = 0.0; xdot_ref4 = 0.15; ydot_ref4 = 0.0
            v_ref4 = np.sqrt(np.power(xdot_ref4, 2) + np.power(ydot_ref4,2)); omega_ref4 = 0.0
            #

        """
        if (t_predict > 10.0 and t_predict <= 20.0):
            x_ref1 = -3.0 + 0.15*t_predict; y_ref1 = +0.5; theta_ref1 = 0.0; xdot_ref1 = 0.15; ydot_ref1 = 0.0
            v_ref1 = np.sqrt(np.power(xdot_ref1,2)+np.power(ydot_ref1,2)); omega_ref1 = 0.0
            #
            x_ref2 = -3.0 + 0.15*t_predict; y_ref2 = 0.0; theta_ref2 = 0.0; xdot_ref2 = 0.15; ydot_ref2 = 0.0
            v_ref2 = np.sqrt(np.power(xdot_ref2,2)+np.power(ydot_ref2,2)); omega_ref2 = 0.0
            #
            x_ref3 = -3.0 + 0.15*t_predict; y_ref3 = +1.5; theta_ref3 = 0.0; xdot_ref3 = 0.15; ydot_ref3 = 0.0
            v_ref3 = np.sqrt(np.power(xdot_ref3, 2) + np.power(ydot_ref3,2)); omega_ref3 = 0.0
            #
            x_ref4 = -3.0 + 0.15*t_predict; y_ref4 = 1.0; theta_ref4 = 0.0; xdot_ref4 = 0.15; ydot_ref4 = 0.0
            v_ref4 = np.sqrt(np.power(xdot_ref4, 2) + np.power(ydot_ref4,2)); omega_ref4 = 0.0
            #


        if (t_predict > 20.0 and t_predict <= 30.0):
            x_ref1 = -3.0 + 0.15*t_predict; y_ref1 = 1.0; theta_ref1 = 0.0; xdot_ref1 = 0.15; ydot_ref1 = 0.0
            v_ref1 = np.sqrt(np.power(xdot_ref1,2)+np.power(ydot_ref1,2)); omega_ref1 = 0.0
            #
            x_ref2 = -3.0 + 0.15*t_predict; y_ref2 = +1.5; theta_ref2 = 0.0; xdot_ref2 = 0.15; ydot_ref2 = 0.0
            v_ref2 = np.sqrt(np.power(xdot_ref2,2)+np.power(ydot_ref2,2)); omega_ref2 = 0.0
            #
            x_ref3 = -3.0 + 0.15*t_predict; y_ref3 = 0.5; theta_ref3 = 0.0; xdot_ref3 = 0.15; ydot_ref3 = 0.0
            v_ref3 = np.sqrt(np.power(xdot_ref3, 2) + np.power(ydot_ref3,2)); omega_ref3 = 0.0
            #
            x_ref4 = -3.0 + 0.15*t_predict; y_ref4 = 0.0; theta_ref4 = 0.0; xdot_ref4 = 0.15; ydot_ref4 = 0.0
            v_ref4 = np.sqrt(np.power(xdot_ref4, 2) + np.power(ydot_ref4,2)); omega_ref4 = 0.0
            #


        if (t_predict > 30.0 and t_predict <= 40.0):
            x_ref1 = -3.0 + 0.15*t_predict; y_ref1 = +1.5; theta_ref1 = 0.0; xdot_ref1 = 0.15; ydot_ref1 = 0.0
            v_ref1 = np.sqrt(np.power(xdot_ref1,2)+np.power(ydot_ref1,2)); omega_ref1 = 0.0
            #
            x_ref2 = -3.0 + 0.15*t_predict; y_ref2 = 0.0; theta_ref2 = 0.0; xdot_ref2 = 0.15; ydot_ref2 = 0.0
            v_ref2 = np.sqrt(np.power(xdot_ref2,2)+np.power(ydot_ref2,2)); omega_ref2 = 0.0
            #
            x_ref3 = -3.0 + 0.15*t_predict; y_ref3 = +1.0; theta_ref3 = 0.0; xdot_ref3 = 0.15; ydot_ref3 = 0.0
            v_ref3 = np.sqrt(np.power(xdot_ref3, 2) + np.power(ydot_ref3,2)); omega_ref3 = 0.0
            #
            x_ref4 = -3.0 + 0.15*t_predict; y_ref4 = 0.5; theta_ref4 = 0.0; xdot_ref4 = 0.15; ydot_ref4 = 0.0
            v_ref4 = np.sqrt(np.power(xdot_ref4, 2) + np.power(ydot_ref4,2)); omega_ref4 = 0.0
            #
        """

        # args['p'][n_sc*k+3:n_sc*k+6] = np.array([[x_ref], [y_ref], [theta_ref]])
        # args['p'][n_sc*k+6:n_sc*k+8] = np.array([[v_ref], [omega_ref]])
        args['p'][(k+1)*n_states+k*n_controls:(k+2)*n_states+k*n_controls] = \
        np.array([[x_ref1], [y_ref1], [theta_ref1], [x_ref2], [y_ref2], [theta_ref2], \
        [x_ref3], [y_ref3], [theta_ref3], [x_ref4], [y_ref4], [theta_ref4]])
        args['p'][(k+2)*n_states+k*n_controls:(k+2)*n_states+(k+1)*n_controls] = \
        np.array([[v_ref1], [omega_ref1], [v_ref2], [omega_ref2], \
        [v_ref3], [omega_ref3], [v_ref4], [omega_ref4]])
    ...
    # print(time.clock(), u_cl[mpciter,0], u_cl[mpciter,1], u_cl[mpciter,2], u_cl[mpciter,3], u_cl[mpciter,4], u_cl[mpciter,5])

    print(np.round(time.clock(), 3), np.round(u_cl[mpciter,0], 3), np.round(u_cl[mpciter,1], 3), \
    np.round(u_cl[mpciter,2], 3), np.round(u_cl[mpciter,3], 3),\
    np.round(u_cl[mpciter,4], 3), np.round(u_cl[mpciter,5], 3), \
    np.round(u_cl[mpciter,6], 3), np.round(u_cl[mpciter,7], 3))

    # print(np.round(time.clock(), 3), np.round(Xr1[0,0], 3), np.round(Xr1[1,0], 3), \
    # np.round(Xr2[0,0], 3), np.round(Xr2[1,0], 3),\
    # np.round(Xr3[0,0], 3), np.round(Xr3[1,0], 3), np.round(Xr4[0,0], 3), np.round(Xr4[1,0], 3))
